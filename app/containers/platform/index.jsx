...
import { Select, Row, Col, Empty, List, notification } from 'antd';
...
const Overview = ({ curOrg, orgs }) => {
  ...
  let CHART = useRef([
    {
      key: 'platform_prvider_env_count_hold',
      domRef: platform_prvider_env_count_hold,
      ins: null,
    },
    {
      key: 'platform_prvider_resource_count_hold',
      domRef: platform_prvider_resource_count_hold,
      ins: null,
    },
    {
      key: 'platform_prvider_resource_type_hold',
      domRef: platform_prvider_resource_type_hold,
      ins: null,
    },
    {
      key: 'platform_resource_change_trend',
      domRef: platform_resource_change_trend,
      ins: null,
    },
    {
      key: 'platform_number_of_active_resources',
      domRef: platform_number_of_active_resources,
      ins: null,
    },
  ]);
  ...
  const fetchDetail = async () => {
    const detailData = await Promise.all([
      getStat(selectedOrganization.join(',')),
      getProviderEnv(selectedOrganization.join(',')),
      getProviderResource(selectedOrganization.join(',')),
      getProviderType(selectedOrganization.join(',')),
      getProviderWeek(selectedOrganization.join(',')),
      getProviderActive(selectedOrganization.join(',')),
    ]);
    const list = [
      'stat',
      'providerEnv',
                ...
                <div className={styles.statisticsCard}>
                  ...
                  <div className={styles.statisticsBottom}>
                    <div>
                      <span>总数</span>
                      <span>
                        {((data.stat || {})[item] || {}).total} <span>/</span>
                      </span>
                    </div>{' '}
                    <div>
                      <span>活跃</span>
                ...
                <div className={classNames(styles.content)}>
                  <div>
                    <span className={styles.content_title}>
                      {t('define.charts.platform_prvider_env_count_hold')}
                    </span>
                    <>
                      ...
                      <div
                        ref={platform_prvider_env_count_hold}
            ...
            <Col span={8}>
              <div className={styles.env_state}>
                <div className={classNames(styles.content)}>
                  <div>
                    <span className={styles.content_title}>
                      {t('define.charts.platform_prvider_resource_count_hold')}
                    </span>
                    <>
                      ...
                      <div
                        ref={platform_prvider_resource_count_hold}
                        style={{ width: '100%', height: 214 }}
                      ></div>
                    </>
                  </div>
            ...
            <Col span={8}>
              <div className={styles.resource_type}>
                ...
                <div className={classNames(styles.content)}>
                  <div>
                    <span className={styles.content_title}>
                      {t('define.charts.platform_prvider_resource_type_hold')}
                    </span>
                    <>
                      ...
                      <div
                        ref={platform_prvider_resource_type_hold}
                        style={{ width: '100%', height: 214 }}
                      ></div>
                    </>
                  </div>
              ...
              <div className={styles.env_state}>
                <div className={classNames(styles.content)}>
                  <span
                    ...
                    style={{ marginLeft: 20 }}
                  >
                    {t('define.page.platform_resource_change_trend')}
                  </span>
                  <div style={{ width: '100%', height: '100%' }}>
                    {isEmpty(data.providerWeek) ? (
                      ...
                      <div
                        ref={platform_resource_change_trend}
              ...
              <div className={styles.env_state}>
                <div className={classNames(styles.content)}>
                  <span
                    ...
                    style={{ marginLeft: 20 }}
                  >
                    {t('define.page.platform_number_of_active_resources')}
                  </span>
                  <div style={{ width: '100%', height: '100%' }}>
                    {isEmpty(data.providerActive) ? (
                      ...
                      <div
                        ref={platform_number_of_active_resources}
                        style={{ width: '100%', height: '100%' }}
                      ></div>
                    )}
                  </div>
          ...
          renderItem={item => (
            <List.Item className={styles.listbody}>
              <div className={styles.dynamicTitle}>
                ...
                <EllipsisText style={{ maxWidth: 200, paddingLeft: 8 }}>{`${
                  item.actionName || '-'
                }${item.objectName && '：'} ${item.objectName} `}</EllipsisText>
              </div>
              <div className={styles.orgInfo}>
                <span>{moment(item.createdAt).format('MM-DD HH:mm:ss')}</span>{' '}
