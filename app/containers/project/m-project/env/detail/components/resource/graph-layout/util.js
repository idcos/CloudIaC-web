  ...
  try {
    ...
    const getAllResourcesList = children => {
      ...
      (children || []).forEach(item => {
        ...
        const resourcesList = (item.resourcesList || []).filter(
          filterByKeyword,
        );
        if (resourcesList.length > 0) {
          ...
          allResourcesList = [
            ...allResourcesList,
            ...getAllResourcesList(itemChildren),
          ];
        }
      });
    ...
    const formatTreeData = data => {
      data = data || {};
      const resourcesList = isEmpty(data.resourcesList)
        ? getAllResourcesList(data.children)
        : data.resourcesList.filter(filterByKeyword);
    ...
    const filterChildren = children => {
      const newChildren = (children || []).filter(
        it => (it.resourcesList || []).length > 0,
      );
      newChildren.forEach(
        it => it.children && (it.children = filterChildren(it.children)),
      );
      return newChildren;
    };
    data = cloneDeep(data) || {};
