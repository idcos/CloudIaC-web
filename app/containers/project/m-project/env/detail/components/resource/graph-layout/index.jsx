...
const GraphLayout = ({ setMode }) => {
  ...
  const {
    ...
    () => {
      const resourcesApis = {
        env: envAPI.getResourcesGraphList.bind(null, {
          orgId,
          projectId,
          envId,
          dimension,
        }),
        task: taskAPI.getResourcesGraphList.bind(null, {
          orgId,
          projectId,
          taskId,
          dimension,
  ...
  return (
    <div ref={ref} className={styles.graphLayout}>
      <Row justify='space-between' className={styles.header}>
        <Space>
          ...
          <Select
            ...
            style={{ width: 153 }}
            options={Object.keys(DIMENSION_ENUM).map(key => ({
              label: DIMENSION_ENUM[key],
              value: key,
            }))}
          />
