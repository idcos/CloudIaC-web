...
import { Descriptions, Empty, Divider, Tooltip } from 'antd';
...
const EnvInfo = () => {
  ...
  const getUrlMap = async () => {
    ...
    await linkToPage({
      path: (envInfo.workdir + '/' + envInfo.tfVarsFile)
        .replace('//', '/')
        .replace('///', '/'),
    }).then(tfVarsFileUrl => {
    ...
    await linkToPage({
      path: (envInfo.workdir + '/' + envInfo.playbook)
        .replace('//', '/')
        .replace('///', '/'),
    }).then(playbookUrl => {
  ...
  return (
    <div className={'envInfoBox'}>
        ...
        <div className='info-border'>
          ...
          >
            {envInfo.isBilling && (
              ...
              >
                {envInfo.monthCost ? envInfo.monthCost.toFixed(2) : 0}
                {t('define.money.yuan')}
              </Descriptions.Item>
            )}
            <Descriptions.Item
              span={3}
              label={t('define.env.field.resourcesNum')}
            >
              {envInfo.resourceCount}
            </Descriptions.Item>
          </Descriptions>
          ...
          <Descriptions
            ...
            contentStyle={{ color: '#57606A' }}
          >
            <Descriptions.Item span={3} label={t('define.env.field.lifeTime')}>
              {formatTTL(envInfo)}
            </Descriptions.Item>
            <Descriptions.Item span={3} label={t('define.updateTime')}>
              {timeUtils.format(envInfo.updatedAt) || '-'}
            </Descriptions.Item>
          </Descriptions>
          <Divider style={{ marginTop: 0, marginBottom: '18px' }} />
          <Descriptions
            ...
            contentStyle={{ color: '#57606A' }}
          >
            <Descriptions.Item span={3} label={t('define.scope.template')}>
              <span
                onClick={() => {
                  history.push(
                    `/org/${orgId}/project/${projectId}/m-project-ct?name=${envInfo.templateName}`,
                  );
                }}
                className={styles.linkToPage}
              >
                {envInfo.templateName || '-'}
              </span>
            </Descriptions.Item>
            <Descriptions.Item
              span={3}
            ...
            <Descriptions.Item span={3} label={t('define.workdir')}>
              {envInfo.workdir || '-'}
            </Descriptions.Item>
            ...
            <Descriptions.Item span={3} label='Commit ID'>
              {taskInfo.commitId ? (
                urlMap.commitUrl ? (
                  <span
                    onClick={() => window.open(urlMap.commitUrl)}
                    className={styles.linkToPage}
                  >
          ...
          <Descriptions
            ...
            contentStyle={{ color: '#57606A' }}
          >
            <Descriptions.Item span={3} label={t('define.ssh')}>
              {envInfo.keyName || '-'}
            </Descriptions.Item>
            <Descriptions.Item span={3} label={t('define.variable.tfVarsFile')}>
              {envInfo.tfVarsFile ? (
                urlMap.tfVarsFileUrl ? (
                  <span
                    onClick={() => window.open(urlMap.tfVarsFileUrl)}
                    className={styles.linkToPage}
                  >
                    {envInfo.tfVarsFile}
                  </span>
            ...
            <Descriptions.Item span={3} label={t('define.variable.playbook')}>
              {envInfo.playbook ? (
                urlMap.playbookUrl ? (
                  <span
                    onClick={() => window.open(urlMap.playbookUrl)}
                    className={styles.linkToPage}
                  >
                    {envInfo.playbook}
                  </span>
          ...
          <Descriptions
            ...
            contentStyle={{ color: '#57606A' }}
          >
            <Descriptions.Item span={3} label={t('define.env.field.runner')}>
              {envInfo.runnerId || '-'}
            </Descriptions.Item>
            <Descriptions.Item span={3} label='Target'>
              {envInfo.target || '-'}
            </Descriptions.Item>
            <Descriptions.Item
              span={3}
            ...
            >
              {(envInfo.triggers || []).includes('commit')
                ? t('define.yes')
                : t('define.no')}
            </Descriptions.Item>
            <Descriptions.Item
              span={3}
              label={t('define.env.field.triggers.prmr')}
            >
              {(envInfo.triggers || []).includes('prmr')
                ? t('define.yes')
                : t('define.no')}
            </Descriptions.Item>
          </Descriptions>
      ...
      <div
        className={classNames('cost-report', {
          isBillingStyle: !envInfo.isBilling,
        })}
      >
        <div className='cost-report-border'>
          {envInfo.isBilling ? (
            ...
            <Empty
              ...
              description={t('define.noOpenCostStatistics')}
              style={{ marginTop: 200 }}
            />
          )}
        </div>
