...
const CCTList = () => {
  ...
  const { run: changeEnabled } = useRequest(
    params =>
      requestWrapper(ctplAPI.enabled.bind(null, params), { autoSuccess: true }),
    {
      manual: true,
  ...
  const { run: runScan, fetches: scanFetches } = useRequest(
    ({ id }) => requestWrapper(ctplAPI.runScan.bind(null, { tplId: id })),
    {
      manual: true,
      fetchKey: params => params.id,
  ...
  const {
    ...
    run: fetchList,
    refresh: refreshList,
  } = useRequest(params => requestWrapper(ctplAPI.list.bind(null, params)), {
    manual: true,
    onSuccess: data => {
      check({
        list: data.list || [],
  ...
  // 表单搜索和table关联hooks
  const { tableProps, onChangeFormParams } = useSearchFormAndTable({
    tableData,
    onSearch: params => {
  ...
  const switchEnabled = ({ enabled, id, policyGroups, name }) => {
    if (enabled) {
      openBindPolicyGroupModal(
        { id, policyGroups, title: t('define.ct.field.policyEnable') },
        () => {
      ...
      render: (policyEnable, record) => {
        ...
        return (
          <Switch
            ...
            size='small'
            onChange={checked =>
              switchEnabled({ enabled: checked, id, policyGroups, name })
            }
          />
        );
      },
  ...
  return (
    <Layout
      extraHeader={
        <PageHeader title={t('define.scope.template')} breadcrumb={true} />
      }
    >
