...
const CenvList = () => {
  ...
  const { data: projectOptions = [] } = useRequest(() =>
    requestWrapper(projectAPI.allEnableProjects.bind(null, {}), {
      formatDataFn: res =>
        ((res.result || {}).list || []).map(it => ({
          label: it.name,
          value: it.id,
        })),
    }),
  ...
  // 启用/禁用Stack扫描
  const { run: changeEnabled } = useRequest(
    params =>
      requestWrapper(cenvAPI.enabled.bind(null, params), { autoSuccess: true }),
    {
      manual: true,
  ...
  // 合规检测
  const { run: runScan, fetches: scanFetches } = useRequest(
    ({ id }) => requestWrapper(cenvAPI.runScan.bind(null, { envId: id })),
    {
      manual: true,
      fetchKey: params => params.id,
  ...
  const {
    ...
    run: fetchList,
    refresh: refreshList,
  } = useRequest(params => requestWrapper(cenvAPI.list.bind(null, params)), {
    manual: true,
    onSuccess: data => {
      check({
        list: data.list || [],
  ...
  const openBindPolicyGroupModal = (
    { id, title, tplId, policyGroups },
    onSuccess = refreshList,
  ) => {
  ...
  const switchEnabled = ({ enabled, id, tplId, policyGroups, name }) => {
    if (enabled) {
      openBindPolicyGroupModal(
        { id, tplId, policyGroups, title: t('define.ct.field.policyEnable') },
        () => {
      ...
      Modal.confirm({
        ...
        content: `${t(
          'define.closeComplianceScan.confirm.content.prefix',
        )}${name}${t('define.closeComplianceScan.confirm.content.suffix')}`,
      ...
      render: (policyEnable, record) => {
        ...
        return (
          <Switch
            ...
            size='small'
            onChange={checked =>
              switchEnabled({ enabled: checked, id, tplId, policyGroups, name })
            }
          />
        );
      },
  ...
  return (
    <Layout
      extraHeader={
        <PageHeader title={t('define.scope.env')} breadcrumb={true} />
      }
    >
      ...
      {bindPolicyGroupModalProps.visible && (
