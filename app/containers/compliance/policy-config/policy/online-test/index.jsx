...
const OnlineTest = ({ match = {} }) => {
  ...
  const { run: fetchInput, loading: fetchInputLoading } = useRequest(
    () => requestWrapper(policiesAPI.parse.bind(null, parseParams)),
    {
      manual: true,
  ...
  const { data: ctOptions, run: fetchCtOptions } = useRequest(
    () =>
      requestWrapper(ctplAPI.list.bind(null, { pageSize: 0 }), {
        formatDataFn: res =>
  ...
  const { data: envOptions, run: fetchEnvOptions } = useRequest(
    () =>
      requestWrapper(cenvAPI.list.bind(null, { pageSize: 0 }), {
        formatDataFn: res =>
          ((res.result || {}).list || []).map(it => ({
            label: it.name,
            value: it.id,
  ...
  const {
    ...
    () => requestWrapper(policiesAPI.test.bind(null, { input, rego })),
    {
      manual: true,
      formatResult: res => ({
  ...
  const { loading: pageLoading } = useRequest(
    () => requestWrapper(policiesAPI.detail.bind(null, policyId)),
    {
      ready: !!policyId,
      onSuccess: res => {
        const { rego } = res || {};
  ...
  const goPolicyListPage = () =>
    history.push(`/org/${orgId}/compliance/policy-config/policy`);
  ...
  const TestStatus = useMemo(() => {
    const map = {
      passed: (
        <CustomTag type='success' text={t('define.scan.status.passed')} />
      ),
      violated: (
        <CustomTag type='error' text={t('define.scan.status.violated')} />
      ),
  ...
  return (
    <Layout
      extraHeader={
        <PageHeader title={t('define.onlineTest')} breadcrumb={true} />
      }
    >
      <div className='idcos-card'>
        <Spin spinning={pageLoading}>
          ...
          <AffixBtnWrapper align='right'>
            <Button onClick={goPolicyListPage}>
              {t('define.action.close')}
            </Button>
            <Button onClick={test} type='primary' loading={testLoading}>
              {t('define.onlineTest')}
            </Button>
          </AffixBtnWrapper>
        </Spin>
