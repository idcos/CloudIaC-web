...
const Policy = ({ location, match }) => {
  ...
  const {
    ...
    params => requestWrapper(policiesAPI.list.bind(null, params)),
    {
      manual: true,
    },
  );
  ...
  const goOnlineTestPage = id => {
    history.push(
      `/org/${orgId}/compliance/policy-config/policy/online-test/${id || ''}`,
    );
  };
  ...
  const columns = [
    {
      ...
      ellipsis: true,
      render: (text, record) => (
        <a onClick={() => onOpenDetailsDrawer(record.id)}>{text}</a>
      ),
    },
    {
      dataIndex: 'tags',
    ...
    <Layout
      extraHeader={
        <PageHeader
          ...
          breadcrumb={true}
          subDes={
            <Button onClick={() => goOnlineTestPage()}>
              {t('define.onlineTest')}
            </Button>
          }
        />
            ...
            <Col>
              <Space>
                <Select
                  ...
                  allowClear={true}
                  placeholder={t(
                    'define.policy.search.policyGroup.placeholder',
                  )}
                ...
                <Select
                  style={{ width: 264 }}
                  allowClear={true}
                  options={Object.keys(POLICIES_SEVERITY_STATUS_ENUM).map(
                    it => ({
                      label: POLICIES_SEVERITY_STATUS_ENUM[it],
                      value: it,
                    }),
                  )}
