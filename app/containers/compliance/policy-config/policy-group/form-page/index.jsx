...
const steps = [
  {
    type: 'source',
    title: t('define.policyGroup.form.step.source'),
    content: <Source />,
  },
  {
    type: 'setting',
    title: t('define.policyGroup.form.step.setting'),
    content: <Setting />,
  },
];
...
const FormPage = ({ match = {} }) => {
  ...
  const { run: fetchDetail } = useRequest(
    () => requestWrapper(cgroupsAPI.detail.bind(null, { policyGroupId })),
    {
      manual: true,
      onSuccess: data => {
  ...
  // 创建策略组
  const { loading: createLoading, run: create } = useRequest(
    params =>
      requestWrapper(cgroupsAPI.create.bind(null, params), {
        autoSuccess: true,
        ...
        <div className={styles.formPage}>
          <Steps current={current}>
            {steps.map((item, index) => (
              <Step
                style={{ cursor: policyGroupId ? 'pointer' : 'default' }}
                key={item.type}
                title={item.title}
