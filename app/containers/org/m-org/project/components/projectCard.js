...
const ProjectCard = ({
  ...
  return (
    <div
      className={`pjtItemBox ${!!isLastUse ? 'recent-project' : ''}`}
      onClick={() => item.status === 'enable' && changeProject(item.id)}
    >
      {!!isLastUse && (
        <div className='last-use-bar' role='img'>
          <img
            ...
            src={formatImgUrl(`/assets/img/recent-project-${language}.png`)}
          />
        </div>
      )}
        ...
        <span className={'pjtItemBox-header-left'}>
          <div role='img'>
            <img
              width={18}
              height={20}
              src={formatImgUrl('/assets/img/jewel.png')}
            />
          </div>
          <div className={'pjtItemBox-header-left-name'}>
            <EllipsisText>{item.name}</EllipsisText>
          </div>
          {!!item.activeEnvironment && (
            <Tooltip title={t('define.activeEnvironment')}>
              <div className={'pjtItemBox-header-left-count'}>
                {item.activeEnvironment}
              </div>
            </Tooltip>
          )}
        ...
        {!readOnly && (
          <div className='pjtItemBox-actions'>
            <Tooltip title={t('define.action.modify')}>
              <Button
                icon={<RenameIcon style={{ fontSize: 14, color: '#008C5A' }} />}
                onClick={e => handleActions({ key: 'modify', domEvent: e })}
                type='text'
                size='small'
              />
            </Tooltip>
            {item.status === 'enable' ? (
              <Tooltip title={t('define.project.status.disabled')}>
                <Button
                  icon={<GdIcon style={{ fontSize: 14, color: '#008C5A' }} />}
                  onClick={e => handleActions({ key: 'disabled', domEvent: e })}
                  type='text'
                  size='small'
                />
              </Tooltip>
              ...
              <Tooltip title={t('define.project.action.recovery')}>
                <Button
                  icon={<GdIcon style={{ fontSize: 14, color: '#008C5A' }} />}
                  onClick={e => handleActions({ key: 'recovery', domEvent: e })}
                  type='text'
                  size='small'
                />
              </Tooltip>
      ...
      <div className={'project-report'}>
        <div
          ...
          style={{ display: isEmpty(item.resStats) ? '' : 'none' }}
          role='img'
        >
          <img
            width={58}
            height={58}
            src={formatImgUrl('/assets/img/no-data.png')}
          />
          <div className='empty-text'>{t('define.noDataView')}</div>
        </div>
        <div
          ref={project_trend_Line}
          style={{
            display: isEmpty(item.resStats) ? 'none' : '',
            width: '100%',
            height: '100%',
            opacity: '0.44',
          }}
