...
const FormModal = ({ orgId, event$ }) => {
  ...
  // 查询详情
  const { loading: detailLoading, run: fetchFormDetail } = useRequest(
    id => requestWrapper(varGroupAPI.detail.bind(null, { orgId, id })),
    {
      onSuccess: detail => {
  ...
  // 创建
  const { loading: createLoading, run: createResourceAccount } = useRequest(
    params =>
      requestWrapper(
        varGroupAPI.create.bind(null, {
          orgId,
          type: 'environment',
          ...params,
        }),
  ...
  // 更新
  const { loading: updateLoading, run: updateResourceAccount } = useRequest(
    params =>
      requestWrapper(
        varGroupAPI.update.bind(null, {
          orgId,
          type: 'environment',
          id,
          ...params,
  ...
  const onOk = async () => {
    const { name, variables, projectIds, provider, costCounted } =
  ...
  return (
    <Modal
      ...
      title={
        id
          ? t('define.resourceAccount.action.modify')
          : t('define.resourceAccount.action.add')
      }
      onCancel={onClose}
      onOk={onOk}
    ...
    >
      <Spin spinning={detailLoading}>
        <Form form={form} {...FL}>
                ...
                <>
                            ...
                            () => ({
                              validator(_, value) {
                                return new Promise((resolve, reject) => {
                                  ...
                                  const filterList = variables.filter(
                                    ({ name }) => name === value,
                                  );
                                  if (filterList.length > 1 && value) {
                                    reject(
                                      new Error(
                                        t('define.variable.sameKeyError'),
                                      ),
                                    );
                                  }
                                  resolve();
                  ...
                  <Form.Item>
                    ...
                    >
                      <PlusOutlined />
                      <span>
                        {t('define.resourceAccount.action.addKeyValue')}
                      </span>
                    </Space>
                  </Form.Item>
                </>
          ...
          <Form.Item name={'projectIds'} label={t('define.bindProject')}>
            <Select
