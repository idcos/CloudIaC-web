...
const Index = ({ reload, toggleVisible, orgId }) => {
  ...
  const onOk = async () => {
    ...
    if (res.code !== 200) {
      ...
      setImportSuccessInfo(res.result || {});
      reload();
    }
  };
  ...
  return <Modal
    title={`${isImportSuccess ? '导入成功' : '导入云模板'}`}
    visible={true}
    ...
    width={573}
    onOk={isImportSuccess ? toggleVisible : onOk}
  >
    {isImportSuccess ? (
      <Space direction='vertical'>
        {Object.keys(importSuccessInfo).map(it => {
          return (importSuccessInfo[it].templates || []).map((dt) => {
            return (<span>
              {!!infoType[it] && <>
