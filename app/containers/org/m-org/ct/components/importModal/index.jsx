...
const Index = ({ reload, toggleVisible, orgId }) => {
  ...
  const computeCount = () => {
    ...
    Object.keys(importInfo).forEach((it, i) => {
      if (
        successTypeList.includes(it) &&
        (it === 'created' || it === 'copied')
      ) {
  ...
  const computeHasColon = data => {
    ...
    Object.keys(data).forEach((it, i) => {
      if (
        (data[it].templates || []).length > 0 ||
        (data[it].varGroups || []).length > 0 ||
        (data[it].vcs || []).length > 0
      ) {
        flag = true;
      }
      setHasColon(flag);
  ...
  const onOk = async () => {
    if (fileList.length === 0) {
      return notification.error({
        message: t('define.ct.import.message.error.selectFile'),
      });
    }
    if (!type) {
      return notification.error({
        message: t('define.ct.import.message.error.actionModeWhenRepeating'),
      });
    }
    ...
    switch (importStatus) {
        ...
        return (
          <Space direction='vertical' size={4}>
            <div className={styles.resultHeader}>
              {t('define.ct.import.success.prefix')}&nbsp;
              {computeCount('success')}&nbsp;
              {t('define.ct.import.success.suffix')}
            </div>
                  ...
                  <>
                    {!!infoType[it] && (
                      <span>
                        ...
                        >
                          {' '}
                          {infoType[it]}【{t('define.scope.template')}】
                        </span>
                        :{' '}
                        <span>
                          <span className={styles.resultText}>{dt.name}</span>
                          <span className={styles.resultText}>({dt.id})</span>
                        </span>
                      </span>
                    )}
                  </>
                  ...
                  <>
                    {!!infoType[it] && (
                      <span>
                        ...
                        >
                          {' '}
                          {infoType[it]}【{t('define.resourceAccount.title')}】
                        </span>
                        :{' '}
                        <span>
                          <span className={styles.resultText}>{dt.name}</span>
      ...
      case 'error':
        return (
          <Space direction='vertical' size={4}>
            <div className={styles.resultHeader}>
              {t('define.ct.import.error.title')}
              {hasColon ? '：' : '。'}
            </div>
            {Object.keys(importInfo).map(it => {
              return (importInfo[it].templates || []).map(dt => {
                  ...
                  <>
                    {!!infoErrorType[it] && (
                      <span>
                        ...
                        >
                          {' '}
                          {infoErrorType[it]}【{t('define.scope.template')}】
                        </span>
                        : <span className={styles.resultText}>{dt.id}</span>
                      </span>
                    )}
                    ...
                    {!!infoErrorType[it] && (
                      <span>
                        ...
                        >
                          {' '}
                          {infoErrorType[it]}【
                          {t('define.resourceAccount.title')}】
                        </span>
                        :{' '}
                        <span>
                          <span className={styles.resultText}>{dt.name}</span>
      ...
      case 'init':
        return (
          <Space direction='vertical' size='middle' style={{ width: '100%' }}>
              ...
              <Upload {...props}>
                ...
                >
                  {fileList[0] && fileList[0]
                    ? t('define.ct.import.init.upload.reSelectFile')
                    : t('define.ct.import.init.upload.selectFile')}
                </Button>
              </Upload>
              <span
                className={styles.uploadExtra}
                style={{
                  marginLeft: 0,
                  color: fileList[0] && fileList[0] ? '#08857C' : 'none',
                }}
              >
                {fileList[0] && fileList[0]
                  ? fileList[0].name
                  : t('define.ct.import.init.upload.extra')}
              </span>
            </Space>
            <Space direction='vertical' size={6} style={{ width: '100%' }}>
              <div className={styles.importHeader}>
                <span>*</span>{' '}
                {t('define.ct.import.init.actionModeWhenRepeating')}
              </div>
              <Radio.Group
                ...
                <Space direction='vertical' size={4} style={{ width: '100%' }}>
                  <Radio value={'update'}>
                    ...
                    <span
                      ...
                      title={t(
                        'define.ct.import.init.actionModeWhenRepeating.updated',
                      )}
                    >
                      {t(
                        'define.ct.import.init.actionModeWhenRepeating.updated',
                  ...
                  <Radio value={'skip'}>
                    ...
                    <span
                      className={styles.radioText}
                      title={t(
                        'define.ct.import.init.actionModeWhenRepeating.skip',
                      )}
                    >
                  ...
                  <Radio value={'copy'}>
                    ...
                    <span
                      ...
                      className={styles.radioText}
                      title={t(
                        'define.ct.import.init.actionModeWhenRepeating.copy',
                      )}
                    >
                  ...
                  <Radio value={'abort'}>
                    ...
                    <span
                      className={styles.radioText}
                      title={t(
                        'define.ct.import.init.actionModeWhenRepeating.abort',
                      )}
                    >
            ...
            <Space style={{ width: '100%' }}>
              <span>{t('define.ct.import.init.associatedProject')}</span>
              <Select
    ...
    <Modal
      title={`${
        importStatus === 'success'
          ? t('define.ct.import.result.success')
          : importStatus === 'error'
          ? t('define.ct.import.result.error')
          : t('define.ct.import')
      }`}
      visible={true}
      ...
      footer={
        <>
          {importStatus === 'init' ? (
            <Space>
              <Button className='ant-btn-tertiary' onClick={toggleVisible}>
                {t('define.ct.import.action.cancel')}
              </Button>
              <Button type={'primary'} onClick={onOk} loading={submitLoading}>
                {t('define.ct.import.action.ok')}
              </Button>
            </Space>
          ) : (
            <Button type={'primary'} onClick={toggleVisible}>
              {t('define.ct.import.action.gotIt')}
            </Button>
          )}
        </>
      }
