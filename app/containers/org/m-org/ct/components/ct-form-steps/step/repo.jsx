...
const Repo = ({
  ...
  const {
    ...
    ({ vcsId, repoRevision, repoId, workdir }) =>
      requestWrapper(
        tplAPI.autotfversion.bind(null, {
          orgId,
          repoId,
          vcsBranch: repoRevision,
          vcsId,
          workdir,
        }),
      ),
  ...
  const onValuesChange = (changedValues, allValues) => {
    ...
    if (changedValues.repoId) {
      ...
      mutateAutoMatchTfVersion(undefined);
      form.setFieldsValue({
        repoFullName: (repos.find(it => it.id === changedValues.repoId) || {})
          .fullName,
        repoRevision: undefined,
        workdir: undefined,
        tfVersion: undefined,
    ...
    if (
      changedValues.repoRevision ||
      changedValues.workdir ||
      changedValues.workdir === ''
    ) {
      mutateAutoMatchTfVersion(undefined);
            ...
            notFoundContent={
              <Empty
                ...
                description={
                  <span>
                    {t('define.noData')}, &nbsp;
                    <a onClick={opVcsModal}>{t('define.vcs.add')}</a>
                  </span>
                }
              />
            }
                ...
                <Col flex='14'>
                  <Form.Item noStyle={true} name='workdir'>
                    <Input
                      ...
                      disabled={!form.getFieldValue('repoRevision')}
                    />
                  </Form.Item>
                </Col>
                <Col flex='4'>
