    ...
    <Collapse expandIconPosition={'right'} forceRender={true}>
      <Collapse.Panel header='环境详情'>
        ...
        >
          ...
          <Descriptions.Item label='分支/标签'>{envInfo.revision || '-'}</Descriptions.Item>
          ...
          <Descriptions.Item label='推送到分支时重新部署'>{(envInfo.triggers || []).includes('commit') ? '是' : '否' }</Descriptions.Item>
          <Descriptions.Item label='PR/MR时执行PLAN'>{(envInfo.triggers || []).includes('prmr') ? '是' : '否' }</Descriptions.Item>
          <Descriptions.Item label='创建人'>{envInfo.creator || '-'}</Descriptions.Item>
          <Descriptions.Item label='创建时间'>{timeUtils.format(envInfo.createdAt) || '-'}</Descriptions.Item>
        </Descriptions>
      </Collapse.Panel>
    </Collapse>
  );
