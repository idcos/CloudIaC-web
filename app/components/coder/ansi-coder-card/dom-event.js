export default class SearchByKeyWord {
  ...
  isFindSameClass(classList = []) {
    const sameClassNameIndex = this.excludeSearchClassNameList.findIndex(
      excludeSearchClassName => {
        return (
          this.findClassListIndex(classList, excludeSearchClassName) !== -1
        );
      },
    );
  ...
  cancelhighLight() {
    ...
    highlightDoms.forEach(highlightDom => {
      ...
      highlightDom.parentNode.replaceChild(
