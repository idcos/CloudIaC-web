...
const PolicyCollapse = ({ data, refresh, targetId, targetType }) => {
  const isError = data.status === 'violated';
  const isFailed = data.status === 'failed';
  const [collapsed, setCollapsed] = useState(true);

  // 更新策略屏蔽
  const { run: updateSuppress } = useRequest(
    () =>
      requestWrapper(
  ...
  const passedList = [
    {
      label: t('policy.detection.info.field.severity'),
      code: 'severity',
      format: text =>
        POLICIES_SEVERITY_STATUS_ENUM[(text || '').toLowerCase()] || text,
    },
    {
      layout: 'vertical',
      code: 'rego',
  ...
  const failedList = [
    ...
    {
      label: t('policy.detection.info.field.resource_type'),
      code: 'resource_type',
    },
    {
      layout: 'vertical',
    ...
    Modal.confirm({
      ...
      content: (
        <div style={{ wordBreak: 'break-all' }}>
          <span>
            {t('policy.suppress.modal.content.prefix')}
            {TARGET_TYPE_ENUM[targetType]}
            {t('policy.suppress.modal.content.middle')}
          </span>
          “<b>{data.policyName || '-'}</b>”
        ...
        <Col className={styles.collapse_header_title}>
          <Space>
            ...
            {data.status === 'violated' && (
              <Button
                className='ant-btn-tertiary'
                disabled={data.policySuppress}
                onClick={onSuppress}
              >
                {t('policy.detection.suppress.btnText')}
              </Button>
            )}
          </Space>
        </Col>
      ...
      {!collapsed && (
        <div className={styles.collapse_body}>
          <Space size='middle' direction='vertical' style={{ width: '100%' }}>
            {(isError ? failedList : isFailed ? errorList : passedList).map(
              ({ label, code, format, layout = 'horizontal' }) => (
                <Descriptions
                  column={1}
                  colon={false}
                  layout={layout}
