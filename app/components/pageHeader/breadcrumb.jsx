...
const breadcrumbNameMap = {
  ...
  'm-org-project': {
    text: `${t('define.orgSet')}: ${t('define.scope.project')}`,
  },
  ...
  'm-project-create': {
    text: `${t('define.projectInfo')}: ${t('define.createProject')}`,
  },
  'm-project-env': {
    text: `${t('define.projectInfo')}: ${t('define.scope.env')}`,
  },
  'm-project-ct': {
    text: `${t('define.projectInfo')}: ${t('define.scope.template')}`,
  },
  'm-project-variable': {
    text: `${t('define.projectInfo')}: ${t('define.variable')}`,
  },
  'm-project-setting': {
    text: `${t('define.projectInfo')}: ${t('define.setting')}`,
  },
      ...
      if (!envId) {
        return (
          <Breadcrumb.Item key={url}>
            <Link to={index === 0 ? '/' : url} disabled={isLastOne}>
              {t('define.deployEnv')}
            </Link>
          </Breadcrumb.Item>
        );
        ...
        return (
          <>
            <Breadcrumb.Item key={envDetailUrl}>
              <Link to={envDetailUrl}>{t('define.envDetail')}</Link>
            </Breadcrumb.Item>
            <Breadcrumb.Item key={url}>
              <Link to={index === 0 ? '/' : url} disabled={isLastOne}>
  ...
  detail: {
    text: t('define.envDetail'),
    indexDiff: 1,
    search: '?tabKey=deployHistory',
  },
  ...
  'policy-group-form': {
    getText: ({ policyGroupId }) =>
      policyGroupId
        ? t('define.modifyPolicyGroup')
...
const BreadcrumbWrapper = ({ location, params }) => {
  ...
  const breadcrumbContent = useMemo(() => {
    const lastOne = pathSnippets
      .filter(
        it => breadcrumbNameMap.hasOwnProperty(it) && breadcrumbNameMap[it],
      )
      .pop();
    return pathSnippets
      .map((snippet, index) => {
        const link = breadcrumbNameMap.hasOwnProperty(snippet)
        ...
        const url = `/${pathSnippets
          .slice(0, index + 1 + (indexDiff || 0))
          .join('/')}${search}`;
        const isLastOne = snippet === lastOne;
        return render ? (
          render({ index, params, isLastOne, url })
        ) : (
          <Breadcrumb.Item key={url}>
            <Link to={index === 0 ? '/' : url} disabled={isLastOne || disabled}>
