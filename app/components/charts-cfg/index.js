  ...
  project_statistics: ({ envActive, envFailed, envInactive } = {}) => {
    return {
      ...
      legend: {
        ...
        data: [
          t('define.charts.project_statistics.activeEnvNum'),
          t('define.charts.project_statistics.failedEnvNum'),
          t('define.charts.project_statistics.inactiveEnvNum'),
        ],
        icon: 'circle',
        selected: [
          { [t('define.charts.project_statistics.activeEnvNum')]: true },
          { [t('define.charts.project_statistics.inactiveEnvNum')]: true },
          { [t('define.charts.project_statistics.failedEnvNum')]: true },
        ],
      },
      series: [
        {
          ...
          data: [
            {
              name: t('define.charts.project_statistics.activeEnvNum'),
              value: envActive || 0,
            },
            {
              name: t('define.charts.project_statistics.inactiveEnvNum'),
              value: envInactive || 0,
            },
            {
              name: t('define.charts.project_statistics.failedEnvNum'),
              value: envFailed || 0,
            },
          ],
          label: {
  ...
  proportion_of_results: inputData => {
    ...
    return {
      title: {
        ...
        subtext: `${names.join('/')}${t(
          'define.charts.proportion_of_results.proportion',
        )}`,
        left: 10,
      },
      color: colors,
  ...
  unsolved_rate: ({ summary = [] }) => {
    ...
    return {
      ...
      legend: {
        ...
        padding: [0, 20, 0, 0],
        data: [
          t('define.charts.unsolved_rate.name.high'),
          t('define.charts.unsolved_rate.name.medium'),
          t('define.charts.unsolved_rate.name.low'),
        ],
        width: 50,
        icon: 'circle',
        formatter: name => {
  ...
  overview_resouces_type: (resStat = []) => {
    ...
    return {
      ...
      series: [
        {
          name: t(
            'define.charts.overview_resouces_type.resoucesTypeProportion',
          ),
          type: 'pie',
          // left: '50%',
          radius: ['50%', '70%'],
    ...
    return {
      ...
      series:
        series_list.map(sery => ({
          ...
          data: dataList.map(item => item.detailsMap[sery]),
        })) || [],
    };
  },
...
export const chartUtils = {
  ...
  update: (chartDescriptor, externalData = {}) => {
    ...
    chartDescriptor.ins =
      chartDescriptor.ins ||
      (domRef.current && echarts.init(domRef.current, 'echarts.idcos.theme'));
