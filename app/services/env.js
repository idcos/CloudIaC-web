import { get, post, put, getWithArgs } from 'utils/xFetch2';
...
const envAPI = {
  ...
  envRedeploy: ({ envId, projectId, orgId, ...resetParams }) => {
    return post(
      `/api/v1/envs/${envId}/deploy`,
      { source: 'manual', ...resetParams },
      { 'IaC-Org-Id': orgId, 'IaC-Project-Id': projectId },
    );
  },
  //销毁环境资源
  ...
  getResourcesGraphList: ({ envId, orgId, projectId, dimension }) => {
    return getWithArgs(
      `/api/v1/envs/${envId}/resources/graph`,
      {
        dimension,
      },
      { 'IaC-Org-Id': orgId, 'IaC-Project-Id': projectId },
  ...
  getResources: ({ orgId, projectId, envId, resourceId }) => {
    return getWithArgs(
      `/api/v1/envs/${envId}/resources/${resourceId}`,
      {},
      {
        'IaC-Org-Id': orgId,
        'IaC-Project-Id': projectId,
  ...
  deployCheck: ({ orgId, projectId, envId, ...resetParams }) => {
    return post(
      `/api/v1/envs/${envId}/deploy/check`,
      { ...resetParams },
      { 'IaC-Org-Id': orgId, 'IaC-Project-Id': projectId },
    );
  },
