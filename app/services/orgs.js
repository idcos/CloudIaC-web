...
const orgsAPI = {
  ...
  inviteUser: ({ orgId, name, email, phone, role }) => {
    return post(
      `/api/v1/orgs/${orgId}/users/invite`,
      {
        name,
        email,
        phone,
  ...
  batchInviteUser: ({ orgId, name, email, phone, role }) => {
    return post(
      `/api/v1/orgs/${orgId}/users/batch_invite`,
      {
        email,
        role,
      },
  ...
  changeOrgUserRole: ({ orgId, id: userId, role }) => {
    return put(
      `/api/v1/orgs/${orgId}/users/${userId}/role`,
      {
        role,
      },
      {
  ...
  orgStatistics: ({ orgId, projectIds, ...restParams }) => {
    return getWithArgs(
      '/api/v1/orgs/projects/statistics',
      { projectIds },
      restParams,
      {
        'IaC-Org-Id': orgId,
